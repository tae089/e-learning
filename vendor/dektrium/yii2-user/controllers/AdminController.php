  ...
  */
  public function actionCreate()
  {
    ...
    if ($user->load(Yii::$app->request->post()) && $user->create()) {
      ...
      $model->mem_date = new \MongoDate($ts);
      $model->username = $user->username;
      $model->level_login = $user->level_login;
      $model->agent_code = $user->agent_code;
      $model->agent_name = $user->agent_name;
      $model->save();
      //return $this->redirect(['update', 'id' => $user->id]);
      return $this->redirect('index.php?r=user/admin/index');
    }
  ...
  */
  public function actionUpdate($id)
  {
    ...
    if ($user->load(Yii::$app->request->post()) && $user->save()) {
      ...
      $model = Member::find()->where(['id_member'=> $user->id])->one();
      $model->username = $user->username;
      $model->level_login = $user->level_login;
      $model->agent_code = $user->agent_code;
      $model->agent_name = $user->agent_name;
  ...
  */
  public function actionUpdateProfile($id)
  {
    ...
    if ($profile->load(Yii::$app->request->post()) && $profile->save()) {
      ...
      //===================================================
      $img = $profile->photo;
      if(UploadedFile::getInstance($profile, 'file')&&$profile->file){  if (!unlink($img)) ;  }
      $profile->file = UploadedFile::getInstance($profile, 'file');
      var_dump($profile->file);
      $save_file = '';
      if($profile->file){
        $imagepath = 'photos/';
        $profile->photo = $imagepath .rand(10,100).'-'.$profile->file->name;
        $save_file = 1;
      }

      if ($profile->save()) {
        if($profile->file){
          $profile->file->saveAs($profile->photo);
        }
        return $this->redirect('index.php?r=user/settings/profile');
      }
  ...
  public function actionDeleteimg($id, $field)
  {
    ...
    $img = $this->finder->findProfileById($id)->$field;
    if($img){
      if (!unlink($img)) {
        return false;
      }
    ...
    $img = $this->finder->findProfileById($id);
    $img->$field = NULL;
    $img->update();
    return $this->redirect('index.php?r=user/settings/profile');
  }
